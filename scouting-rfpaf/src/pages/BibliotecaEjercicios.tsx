...
import {
  Plus, Video, Eye, FileText, Pencil, X, Trash2, Upload, Camera,
} from 'lucide-react'
...
const BibliotecaEjercicios = () => {
  ...
  const uploadImagenRef = useRef<HTMLInputElement>(null)
  const uploadImagenEditRef = useRef<HTMLInputElement>(null)

  const handleImagenUpload = (e: React.ChangeEvent<HTMLInputElement>, setter: (img: string) => void) => {
          ...
          <div className="bg-white rounded-xl shadow-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto flex flex-col">
            ...
            <div className="flex flex-col flex-1 overflow-y-auto">
              <div className="p-5 space-y-4">
                ...
                {/* Imagen subida */}
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1.5">Imagen del ejercicio</label>
                  <div className="flex items-start gap-3">
                    <div className={`flex-shrink-0 w-32 h-20 rounded-lg border-2 overflow-hidden transition-all ${formEj.imagen ? 'border-green-400' : 'border-dashed border-gray-300 flex items-center justify-center bg-gray-50'}`}>
                      {formEj.imagen
                        ? <img src={formEj.imagen} alt="Preview" className="w-full h-full object-cover"/>
                        : <div className="text-center text-gray-400"><Camera className="w-5 h-5 mx-auto mb-1 opacity-40"/><p className="text-[10px]">Sin imagen</p></div>}
                    </div>
                    <div className="flex flex-col gap-2">
                      <input ref={uploadImagenRef} type="file" accept="image/*" className="hidden"
                        onChange={e => handleImagenUpload(e, img => setFormEj(f => ({...f, imagen: img})))}/>
                      <button type="button" onClick={() => uploadImagenRef.current?.click()}
                      ...
                      <p className="text-[10px] text-gray-400 leading-tight">O captura desde la pizarra táctica de abajo.</p>
                    </div>
                  </div>
                </div>
          ...
          <div className="bg-white rounded-xl shadow-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto flex flex-col">
            ...
            <div className="flex flex-col flex-1 overflow-y-auto">
              <div className="p-5 space-y-4">
                ...
                {/* Imagen subida en edición */}
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1.5">Imagen del ejercicio</label>
                  <div className="flex items-start gap-3">
                    <div className={`flex-shrink-0 w-32 h-20 rounded-lg border-2 overflow-hidden transition-all ${formEditEj.imagen ? 'border-green-400' : 'border-dashed border-gray-300 flex items-center justify-center bg-gray-50'}`}>
                      {formEditEj.imagen
                        ? <img src={formEditEj.imagen.startsWith('data:') ? formEditEj.imagen : `data:image/png;base64,${formEditEj.imagen}`} alt="Preview" className="w-full h-full object-cover"/>
                        : <div className="text-center text-gray-400"><Camera className="w-5 h-5 mx-auto mb-1 opacity-40"/><p className="text-[10px]">Sin imagen</p></div>}
                    </div>
                    <div className="flex flex-col gap-2">
                      <input ref={uploadImagenEditRef} type="file" accept="image/*" className="hidden"
                        onChange={e => handleImagenUpload(e, img => setFormEditEj(f => ({...f, imagen: img})))}/>
                      <button type="button" onClick={() => uploadImagenEditRef.current?.click()}
                        className="flex items-center gap-1.5 px-3 py-2 bg-rfpaf-blue text-white text-xs font-semibold rounded-lg hover:bg-rfpaf-blue/90 transition-colors">
                        <Upload className="w-3.5 h-3.5"/> Subir imagen
                      </button>
                      {formEditEj.imagen && (
                        <button type="button" onClick={() => setFormEditEj(f => ({...f, imagen: ''}))}
                          className="flex items-center gap-1 px-3 py-1.5 text-xs text-red-500 border border-red-200 rounded-lg hover:bg-red-50 transition-colors">
                          <X className="w-3 h-3"/> Quitar
                        </button>
                      )}
