...
function TacticalBoard({ onCapture, onRegisterCapture }: TacticalBoardProps) {
  ...
  return (
    <div className="bg-gray-800 rounded-xl p-4 flex flex-col gap-3">
      ...
      {/* Mobile/Tablet layout: Tabs + Single panel */}
      <div className="lg:hidden flex flex-col gap-3">
        {/* Tab buttons */}
        <div className="flex gap-2 border-b border-gray-700">
          <button type="button" onClick={() => setMobilePanel('left')}
            className={`flex-1 px-2 py-2 text-xs font-semibold border-b-2 transition-colors ${mobilePanel === 'left' ? 'border-white text-white' : 'border-transparent text-gray-400 hover:text-gray-300'}`}>
            Herramientas
          </button>
          <button type="button" onClick={() => setMobilePanel('canvas')}
            className={`flex-1 px-2 py-2 text-xs font-semibold border-b-2 transition-colors ${mobilePanel === 'canvas' ? 'border-white text-white' : 'border-transparent text-gray-400 hover:text-gray-300'}`}>
            Campo
          </button>
          <button type="button" onClick={() => setMobilePanel('right')}
            className={`flex-1 px-2 py-2 text-xs font-semibold border-b-2 transition-colors ${mobilePanel === 'right' ? 'border-white text-white' : 'border-transparent text-gray-400 hover:text-gray-300'}`}>
            Controles
          </button>
        </div>
          ...
          <div className="flex flex-col gap-3 max-h-[70vh] overflow-y-auto">
            ...
            <div className="bg-gray-900/60 rounded-xl overflow-hidden border border-white/5 flex-shrink-0">
              ...
              {([1,2,3] as TeamId[]).map(tid=>{
                ...
                return (
                  <div key={tid} className="border-t border-white/5">
                    <button type="button" onClick={()=>toggleTeam(tid)}
                      className="w-full flex items-center gap-2.5 px-3 py-2 hover:bg-white/5 transition-colors">
                      <span className="text-[11px] font-bold px-2.5 py-0.5 rounded-md flex-shrink-0 text-center" style={{backgroundColor:t.bg,color:t.text}}>{t.label}</span>
                      {cnt>0&&<span className="text-[10px] text-white/40">{cnt} en campo</span>}
                      <ChevronDown className={`w-3.5 h-3.5 text-white/35 ml-auto flex-shrink-0 transition-transform duration-200 ${isOpen?'rotate-180':''}`}/>
                    </button>
                    {isOpen&&(
              ...
              <div className="px-3 py-2 border-t border-white/5">
                <button type="button" onClick={()=>setPlacedPlayers([])}
                  className="flex items-center gap-1 text-[10px] text-white/30 hover:text-red-400 transition-colors">
                  <UserX className="w-3 h-3"/> Quitar todas
                </button>
              </div>
            </div>
            ...
            <div className="bg-gray-900/60 rounded-xl overflow-hidden border border-white/5 flex-shrink-0">
              ...
              {placedAccessories.length > 0 && (
                <div className="px-3 pb-2 border-t border-white/5 pt-2">
                  <button type="button" onClick={() => setPlacedAccessories([])}
                    className="flex items-center gap-1 text-[10px] text-white/30 hover:text-red-400 transition-colors">
                    <Eraser className="w-3 h-3"/> Quitar accesorios
                  </button>
                </div>
              )}
            </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            {tool==='text'&&!selPlayer&&(
              <div className="flex gap-2">
                <input type="text" value={textInput} onChange={e=>setTextInput(e.target.value)}
                  onKeyDown={e=>e.key==='Enter'&&handleAddText()}
                  placeholder={textPos?'Escribe y pulsa Enter…':'Haz clic en el campo…'}
                  className="flex-1 bg-white/10 text-white text-xs rounded-lg px-3 py-2 border border-white/20 placeholder-white/40 focus:outline-none focus:border-rfpaf-blue"/>
                {textPos&&<button type="button" onClick={handleAddText} className="px-3 py-1.5 bg-rfpaf-blue text-white text-xs rounded-lg font-medium">Añadir</button>}
              </div>
            )}
