...
interface ShapeEl extends BaseEl {
  tool: 'rect' | 'circle' | 'focus' | 'triangle'
...
export default function PintadoAcciones() {
  ...
  const handleMouseMove = useCallback((e: RME) => {
    if (dragState && tool === 'select') {
      ...
      setElements(prev => prev.map(el => {
        ...
        if (orig.tool === 'rect' || orig.tool === 'circle' || orig.tool === 'focus' || orig.tool === 'triangle') {
  ...
  const duplicateSelected = () => {
    ...
    } else if (el.tool === 'rect' || el.tool === 'circle' || el.tool === 'focus' || el.tool === 'triangle') {
