...
export async function exportSesionPDF(sesion: Sesion) {
  ...
  const h2Style =
    'font-size:13px;color:#1e40af;border-bottom:2px solid #1e40af;padding-bottom:4px;margin-bottom:10px;text-transform:uppercase;letter-spacing:1px;margin-top:0'
  ...
  await html2pdf()
    .set({
      margin: 8,
      filename: `sesion-${sesion.fecha || 'entrenamiento'}.pdf`,
